...
import sys
import io
import dataViz_bib.ML_carto_clustering as ml
...
def set_legend(vue,legende):
    ...
    legende=sorted(legende, key=lambda x: (x is None, str(x)))
...
def display_vue(vue,bat_actif,df_batiment,clients,liste_active,dept=75):
    ...
    # *****************************************
    # df_reduit = dask_geopandas.from_geopandas(df_reduit[var_tooltip+['geometry']+[s.upper() for s in var_tooltip]], npartitions=4)
    # df_reduit=df_reduit.dissolve(vue)
    # df_reduit= gpd.GeoDataFrame(df_reduit)
    # df_reduit.columns =[vue, 'geometry']
...
def perform_clustering(vue,bat_actif,df_batiment,clients,liste_active,dept=75):
    couleurs=set_couleurs()
    display_clustering=False
    ...
    labels=[i[1]['label'] for i in dico_colonnes.items() if i[1]['explicative']=='1']
    var_tooltip=[variables[labels.index(i)] for i in liste_active]
    df_reduit['vue']=ml.kmeans_on_mca_coordinates(ml.mca_batiment(df_reduit[var_tooltip]))
    
    df_reduit['temp']=df_reduit['vue']
    var_tooltip=['vue']+var_tooltip
    

    # *****************************************
    df_reduit=df_reduit[var_tooltip+['geometry','temp']].dissolve(by='temp')
    # *********************************************
    ...
    print('dddd')
    legende=set_legend('Groupes de bâtiments',df_reduit['vue'].unique())
    m.get_root().add_child(legende)
