...
def all_vues():
    ...
    ,'proprietaire': {'taxonomie':'additional','explicative':'0','label':'proprietaire'}
...
def set_couleurs():
    ...
    couleurs=['red','pink','orange','green','teal','yellow',"blue",'indigo',\
       'cyan','violet','gray','lime','grape',\
            'gold','silver','darkgrey','black','khaki','whitesmoke'] 
    return couleurs
...
def get_highlight(couleur):
    ...
    style={'red':'red','pink':'pink','orange':'orange','green':'green','teal':'teal','yellow':'yellow',"blue":"blue",'indigo':'indigo',\
       'cyan':'cyan' ,'violet':'violet','gray':'gray','lime':'lime','grape':'grape',\
            'gold':'gold','silver':'silver','darkgrey':'darkgrey','black':'grey','khaki':'darkkhaki','whitesmoke':'silver'}
...
def marker_proprietaire(bat_du_proprio,geo):
    ...
    # <h5><a href=https://www.stonal.com target=_blank>test</a></h5>
    # <h5><a href=http://127.0.0.1:8000/{bat_du_proprio['proprietaire'].iloc[0]}batiment.html_{bat_du_proprio['batiment_groupe_id'].iloc[i]} target=_blank>+</a></h5>

    for _, r in bat_du_proprio.iterrows(): 
        try:
       
            html=f"""
            <h5>{r.proprietaire}</h5>
            <h5>Batiment Id : {r.batiment_groupe_id}</h5>
            <h5>Nb total de lots : {r.rnc_ope_nb_lot_tot_group:}</h5>
            <h5>Nb total de lots tertiaires : {r.rnc_ope_nb_lot_tertiaire_group}</h5>    
            <h5>Estimation GES : {r.dpe_logtype_classe_estim_ges}</h5>
            <h5>Classe conso : {r.dpe_logtype_classe_conso_ener}</h5>
            <h1 align="right"><a href=http://127.0.0.1:8000/batiment_{r.batiment_groupe_id}.html target=_blank>+</a></h1>
            """
    ...
    #         <h5><a href=http://127.0.0.1:8000/batiment_{batiment}.html target=_blank>+</a></h5>
    #         """


    #         # Func = open("C:/Users/FilipeAfonso/Documents/dataViz/data/test_localhost/letest.html","w")
    #         # Func.write(html)
    #         # Func.close()
...
def display_proprietaire(vue,df_batiment,liste_active,bat_actif,clients,display_clustering):
    # print(vue)
    # print(df_batiment)
    # print(liste_active)
    ...
    print(xactif, yactif)
    m = folium.Map(location=(yactif,xactif),min_zoom=0, max_zoom=18, zoom_start=13, tiles='CartoDB positron')
    ...
    df_reduit=df_batiment[var_tooltip+['proprietaire','geometry']]
    df_reduit.replace([None], 'Manquante', inplace=True)
    print(df_reduit[vue].unique)


   
    feature_group = folium.FeatureGroup(name=vue)
    feature_group2 = folium.FeatureGroup(name='batiments proprietaires')
    # feature_group=[folium.FeatureGroup(name=v,overlay=True) for v in liste_active]
    print(vue)
    print("******* go to process_vue")
    process_vue(vue, df_reduit,liste_active,display_clustering,feature_group)
    ...
    print('legende_vue=',vue)
    print('legende_vue=',all_vues()[vue]['label'])
    feature_group.add_child(set_legend(all_vues()[vue]['label'],df_reduit[vue].unique()))
    
    if(type(clients)==str):
        print(type(clients))
        marker_proprietaire(df_batiment[df_batiment['proprietaire']==clients],feature_group2)
    else :
        popup_batiment(bat_actif,clients,feature_group)
    ...
    # m.add_child(folium.features.ClickForLatLng())
...
def display_parc_immo(vue,df_batiment,liste_active,bat_actif,clients,display_clustering):
    ...
    df_reduit=df_reduit[var_tooltip+['proprietaire','geometry']+[s.upper() for s in var_tooltip]].dissolve(by=var_tooltip)
